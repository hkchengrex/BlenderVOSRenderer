...
class RGBSegWriter(Module):
    ...
    **Configuration**:
    ...
    def __init__(self, config):
        ...
        self.fw_flow_output_key = self.config.get_string("forward_flow_output_key", "forward_flow")
        self.bw_flow_output_key = self.config.get_string("backward_flow_output_key", "backward_flow")
        ...
        self.flw_fw_out_data_dir = os.path.join(base_folder, 'ForwardFlows', vid_name)
        self.flw_bw_out_data_dir = os.path.join(base_folder, 'BackwardFlows', vid_name)

        os.makedirs(self.ren_out_data_dir, exist_ok=True)
        os.makedirs(self.seg_out_data_dir, exist_ok=True)
        os.makedirs(self.flw_fw_out_data_dir, exist_ok=True)
        os.makedirs(self.flw_bw_out_data_dir, exist_ok=True)
    ...
    def run(self):
        ...
        # Find path pattern of flows
        fw_flow_output = self._find_registered_output_by_key(self.fw_flow_output_key)
        if fw_flow_output is None:
            raise Exception("There is no output registered with key " + self.fw_flow_output_key + ". Are you sure you ran the SimFlowRenderer module before?")

        # Find path pattern of flows
        bw_flow_output = self._find_registered_output_by_key(self.bw_flow_output_key)
        if bw_flow_output is None:
            raise Exception("There is no output registered with key " + self.bw_flow_output_key + ". Are you sure you ran the SimFlowRenderer module before?")
        ...
        for frame in range(bpy.context.scene.frame_start, bpy.context.scene.frame_end):
            ...
            # Flow
            source_path = fw_flow_output["path"] % frame
            target_path = os.path.join(self.flw_fw_out_data_dir, os.path.basename(fw_flow_output["path"].replace('%04d','%05d') % (frame)))
            shutil.copyfile(source_path, target_path)

            source_path = bw_flow_output["path"] % frame
            target_path = os.path.join(self.flw_bw_out_data_dir, os.path.basename(bw_flow_output["path"].replace('%04d','%05d') % (frame)))
            shutil.copyfile(source_path, target_path)
            # new_out_paths.append(os.path.basename(target_path))
